...
import { careTaskSchema, petProfileSchema, type GlucoseUnit, type Species } from "../../domain/schema";
import { speciesNouns } from "../../domain/helpers";
...
export function Onboarding() {
  ...
  function finish(event: FormEvent) {
    ...
    const petResult = petProfileSchema.safeParse({
      id: petId, name, species, breed: breed || undefined,
    ...
    const tasks = addTasks ? [
      ...
      id: crypto.randomUUID(), petId, type: "meal_treatment", title, localTime,
  ...
  return (
    <main className="onboarding-shell">
      <section className="onboarding-card">
        ...
        <h1>{step === 0 ? "先说清楚边界" : step === 1 ? "选择宠物类型" : step === 2 ? `认识你的${speciesNouns[species]}` : step === 3 ? "选择记录单位" : "设置今日计划"}</h1>
        <div className="step-dots" aria-label={`第 ${step + 1} 步，共 5 步`}>{[0,1,2,3,4].map((item) => <span key={item} className={item <= step ? "active" : ""} />)}</div>
        <form className="stack" onSubmit={step === 4 ? finish : (event) => { event.preventDefault(); setStep((current) => current + 1); }}>
          {step === 0 && <>
          ...
          {step === 1 && <>
            <p className="lead">犬与猫的照护方式可能不同。产品只展示通用记录字段，不生成物种专属治疗方案。</p>
            <div className="choice-grid">
              {(["dog", "cat"] as const).map((item) => <label className={`choice-card ${species === item ? "selected" : ""}`} key={item}><input type="radio" checked={species === item} onChange={() => setSpecies(item)} />{item === "dog" ? "小狗" : "猫咪"}</label>)}
            </div>
          </>}
          {step === 2 && <>
            <label>{speciesNouns[species]}名字<input autoFocus value={name} onChange={(e) => setName(e.target.value)} placeholder={species === "dog" ? "例如：豆豆" : "例如：团子"} required /></label>
          ...
          <div className="button-row">
            ...
            <button type="submit" disabled={step === 0 && !accepted}>{step === 4 ? "开始记录" : "下一步"}</button>
          </div>
        </form>
      </section>
    </main>
