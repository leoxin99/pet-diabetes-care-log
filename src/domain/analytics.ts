...
export function analyzeRecords(records: CareRecord[], start: Date, end: Date): RecordAnalytics {
  ...
  const recordTypeCounts: Record<RecordType, number> = {
    glucose: 0, meal: 0, insulin: 0, weight: 0, observation: 0, notes: 0,
  };
  ...
  for (const record of records) {
    ...
    if (record.insulinAdministration) recordTypeCounts.insulin += 1;
...
export function recordsToAnalysisCsv(records: CareRecord[]) {
  const headers = [
    ...
    "meal_consumed_level", "water_ml", "insulin_recorded_amount", "insulin_unit",
  ...
  const rows = [...records]
    ...
    .map((record) => [
      ...
      record.waterMl,
      record.insulinAdministration?.recordedAmount,
      record.insulinAdministration?.unitLabel,
      record.weightKg,
